...
class Singer extends Cscms_Controller {
	...
	public function downpic()
	{
            ...
            foreach ($query->result() as $row) {
				...
				if($up=='ok'){
                       //修改数据库
                       $this->db->query("update ".CS_SqlPrefix."singer set pic='/".date('Ym')."/".date('d')."/".$file_name."' where id=".$row->id."");
                       echo "&nbsp;&nbsp;&nbsp;&nbsp;同步<font color=red>".$row->pic."</font>&nbsp;图片成功!&nbsp;&nbsp;新图片名：<a href=\"".piclink('singer','/'.date('Ym').'/'.date('d').'/'.$file_name)."\" target=_blank>".$file_name."</a></br>";
				}else{
                       //修改数据库
