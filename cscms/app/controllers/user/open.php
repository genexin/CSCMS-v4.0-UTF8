...
class Open extends Cscms_Controller {
	...
	public function callback()
	{
			...
			if($uid>0){ //已经存在则登录
                 ...
                 $sqlu="SELECT code,email,pass,sid,yid,id,name,lognum,cion,vip,logtime,viptime FROM ".CS_SqlPrefix."user where id=".$uid."";
	...
	public function log_save()
	{
			...
			if($token!=$_SESSION['token']) msg_url(L('open_16'),'javascript:history.back();');
            ...
            if(empty($username)){
				...
				msg_url(L('open_11'),'javascript:history.back();');  //用户名为空
			}else{
		        ...
		        if(!$row){
	                 ...
	                 if(User_Uc_Mode==1){
                         ...
                         msg_url(L('open_13'),'javascript:history.back();');  //账号不存在
					}
                }else{
						   //修改第三方登录UID
